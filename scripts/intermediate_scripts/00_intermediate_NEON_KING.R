...
# NEON - KING - Fish electrofishing, gill netting, and fyke netting counts (https://doi.org/10.48443/ap3d-rp07)
# SCALES/ SSECR                  
# Sierra Perez   
...
data %>% 
  filter(fishTotalLength == 0) %>% dplyr::count() # 0 zeros
...
unique(substr(data$date, 6, 7)) # months sampled: "04" "10" "11" "06" "05"
...
# most species have low abundances (mean = 33.0), but very right skewed (max = 1845.0)

# some missing taxonRank -> imputing where needed:

data$taxonRank[data$taxonID == "ETHSPE"] <- "species"
data$taxonRank[data$taxonID == "SEMATR"] <- "species"
data$taxonRank[data$taxonID == "ETHNIG1"] <- "species"
data$taxonRank[data$taxonID == "CAMANO"] <- "species"
data$taxonRank[data$taxonID == "CHRERY"] <- "species"
data$taxonRank[data$taxonID == "CAMSPP"] <- "genus"
data$taxonRank[data$taxonID == "GAMAFF"] <- "species"
data$taxonRank[data$taxonID == "CATCOM"] <- "species"
data$taxonRank[data$taxonID == "NOTSP"] <- "genus" # but note that in comment it says that they're emerald shiner (Notropis atherinoides)...?

data %>% 
  filter(taxonRank == "genus" | taxonRank == "family" | taxonRank == "order" | taxonRank == "phylum" |  taxonRank == "class") %>% dplyr::count() 
# 258 observations not ID'd to species 
...
temp_final <- temp_final %>% 
  filter(!YEAR == 2018) # drop 2018 b/c averages low b/c temp data only for end of Oct-Dec of that year
...
plot.presence(intermediate, 
              ...
              subsite = F)
# much higher diversity than other NEON sites
