...
# NEON - LEWI - Fish electrofishing, gill netting, and fyke netting counts (https://doi.org/10.48443/ap3d-rp07)
# SCALES/ SSECR                  
# Sierra Perez   
...
unique(substr(data$date, 6, 7)) # months sampled: "04" "11" "10"
...
# most species have low abundances (mean = 57.09) and somewhat right skewed (max = 473.00)

# some missing taxonRank -> imputing where needed:

data$taxonRank[data$taxonID == "RHIATR"] <- "species"
data$taxonRank[data$taxonID == "COTGIR"] <- "species"
data$taxonRank[data$taxonID == "RHICAT"] <- "species"
data$taxonRank[data$taxonID == "SEMATR"] <- "species"
data$taxonRank[data$taxonID == "CLIFUN"] <- "species"

data %>% 
  filter(taxonRank == "genus" | taxonRank == "family" | taxonRank == "order" | taxonRank == "phylum" |  taxonRank == "class") %>% dplyr::count() 
# 8 observations not ID'd to species 
