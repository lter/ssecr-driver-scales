...
# NEON - PRLA - Fish electrofishing, gill netting, and fyke netting counts (https://doi.org/10.48443/kb2e-va82)
# SCALES/ SSECR                  
# Sierra Perez   
...
## LENGTH --------------------
  ...
  data %>% 
    filter(fishTotalLength == 0) %>% dplyr::count() # 0 length fish
  
  data %>% 
    filter(is.na(fishTotalLength)) %>% dplyr::count() # 19 NA's
...
## DATES --------------------
  ...
  unique(substr(data$date, 6, 7)) # months sampled: "10" "05" "09" "06"
...
## COUNTS/ABUNDANCE --------------------
  ...
  summary(count_check) # mean = 9823.42, max = 60255.00
  ...
  data %>% 
    filter(taxonRank == "genus" | taxonRank == "family" | taxonRank == "order" | taxonRank == "phylum") %>% dplyr::count() 
  # 0 observations only ID'd to family, genus, order, or phylum
...
## SAMPLING EFFORT --------------------
  ...
  # 1 electrofisher 319326
  # 2 gill net      313458
  # 3 mini-fyke net 583537
  # dropping 74% of observations
...
tapply(substr(data$date, 1, 7), list(substr(data$eventID, 1, 16)), timeseries) # unique samplings
  ...
  ggplot(temp, aes(x = DATE, y = surfacewaterTempMean)) +
    geom_point() # missing data for most of 2023, a lot of 2020
  ...
  ggplot(DO, aes(x = DATE, y = dissolvedOxygen)) +
    geom_point() # missing data for most of 2020, all of 2021
...
# (OPTIONAL) DATA VIZ --------------------
  ...
  plot.presence(intermediate, 
                ...
                subsite = F)
  # 5 species only appear in one year
