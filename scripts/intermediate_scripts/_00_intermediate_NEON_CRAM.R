...
# NEON - CRAM - Fish electrofishing, gill netting, and fyke netting counts (https://doi.org/10.48443/ap3d-rp07)
# SCALES/ SSECR                  
# Sierra Perez   
...
## LENGTH --------------------
  ...
  data %>% 
    filter(fishTotalLength == 0) %>% dplyr::count() # no zeros
  
  data %>% 
    filter(is.na(fishTotalLength)) %>% dplyr::count() # 17 NA's
...
## DATES --------------------
  ...
  unique(substr(data$date, 6, 7)) # months sampled: "08" "09" "05" "10" "06"
...
## COUNTS/ABUNDANCE --------------------
  ...
  # most species have low abundances (mean = 43.87), but very right skewed (max = 1566.00)
  
  # some missing taxonRank -> imputing where needed:

  data$taxonRank[data$taxonID == "LEPSP4"] <- "genus"
  data$taxonRank[data$taxonID == "LEPMAC"] <- "species"
  data$taxonRank[data$taxonID == "PERFLA"] <- "species"
  
  data %>% 
    filter(taxonRank == "genus" | taxonRank == "family" | taxonRank == "order" | taxonRank == "phylum" |  taxonRank == "class") %>% dplyr::count() 
  # 756 observations not ID'd to species 
...
# 1 electrofisher  6456
# 2 gill net       4056
# 3 mini-fyke net  7231
# dropping 64% of observations
