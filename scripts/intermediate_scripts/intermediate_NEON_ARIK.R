...
# NEON - ARIK - Fish electrofishing, gill netting, and fyke netting counts (https://doi.org/10.48443/ap3d-rp07)
# SCALES/ SSECR                  
# Sierra Perez   
...
## LENGTH --------------------
  ...
  data %>% 
    filter(fishTotalLength == 0) %>% dplyr::count() # 1 zero
  
  data %>% 
    filter(is.na(fishTotalLength)) %>% dplyr::count() # 9 NA's
...
## DATES --------------------
  ...
  unique(substr(data$date, 6, 7)) # months sampled: Mar, Apr, Oct
...
## COUNTS/ABUNDANCE --------------------
  ...
  # most species have low abundances (mean = 44.72), but very right skewed (max = 932.00)
  ...
  data$taxonRank[data$taxonID == "HYBHAN"] <- "species"
  data$taxonRank[data$taxonID == "LEPCYA"] <- "species"
  data$taxonRank[data$taxonID == "POESP2"] <- "family"
  
  data %>% 
    filter(taxonRank == "genus" | taxonRank == "family" | taxonRank == "order" | taxonRank == "phylum" |  taxonRank == "class") %>% dplyr::count() 
  # 572 observations not ID'd to species 
...
# (OPTIONAL) DATA VIZ --------------------
  ...
  plot.presence(intermediate, 
                ...
                subsite = F)
  # **** 6 species only appear in 1 year *****
