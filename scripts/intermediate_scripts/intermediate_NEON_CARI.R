...
# NEON - CARI - Fish electrofishing, gill netting, and fyke netting counts (https://doi.org/10.48443/ap3d-rp07)
# SCALES/ SSECR                  
# Sierra Perez   
...
## LENGTH --------------------
  ...
  data %>% 
    filter(fishTotalLength == 0) %>% dplyr::count() # no 0s
  
  data %>% 
    filter(is.na(fishTotalLength)) %>% dplyr::count() # 1 NA
...
## DATES --------------------
  ...
  unique(substr(data$date, 6, 7)) # months sampled: May, Jun, Aug, Sep
...
## COUNTS/ABUNDANCE --------------------
  ...
  # very low counts at this site (mean = 9, max = 62)
