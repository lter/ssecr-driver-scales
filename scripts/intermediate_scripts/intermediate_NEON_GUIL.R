...
# NEON - GUIL - Fish electrofishing, gill netting, and fyke netting counts (https://doi.org/10.48443/ap3d-rp07)
# SCALES/ SSECR                  
# Sierra Perez   
...
data %>% 
  filter(fishTotalLength == 0) %>% dplyr::count() # 4 zeros
...
unique(substr(data$date, 6, 7)) # months sampled: "02" "12" "11"
...
# most species have low abundances (mean = 320.0), pretty right skewed (max = 1543.0)

# some missing taxonRank -> imputing where needed:

data$taxonRank[data$taxonID == "POERET"] <- "species"
data$taxonRank[data$taxonID == "XIPHEL"] <- "species"

data %>% 
  filter(taxonRank == "genus" | taxonRank == "family" | taxonRank == "order" | taxonRank == "phylum" |  taxonRank == "class") %>% dplyr::count() 
# 1 observations not ID'd to species 
