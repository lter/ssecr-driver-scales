...
# NEON - LIRO - Fish electrofishing, gill netting, and fyke netting counts (https://doi.org/10.48443/kb2e-va82)
# SCALES/ SSECR                  
# Sierra Perez   
...
data %>% 
  filter(is.na(fishTotalLength)) %>% dplyr::count() # 7 NA's
...
unique(substr(data$date, 6, 7)) # months sampled: "05" "09" "06"
...
summary(count_check) # crazy numbers!! mean = 12.04; max = 126.00

# some missing taxonRank -> imputing where needed:

data$taxonRank[data$taxonID == "UMBLIM"] <- "species"
data$taxonRank[data$taxonID == "MICSAL"] <- "species"
data$taxonRank[data$taxonID == "AMBRUP"] <- "species"

data %>% 
  filter(taxonRank == "genus" | taxonRank == "family" | taxonRank == "order" | taxonRank == "phylum" |  taxonRank == "class") %>% dplyr::count() 
# 1 observation only ID'd to family, genus, order, or phylum -> for "ACTSPP" looks like sometimes they didn't bother sorting PIMPRO & CULINC to species
...
# 1 electrofisher  1096
# 2 gill net        658
# 3 mini-fyke net  1152
# dropping 62% of observations
...
ggplot(temp, aes(x = DATE, y = surfacewaterTempMean)) +
  geom_point() # checking for missing data: drop 2017 because only data for Oct-Dec
...
ggplot(DO, aes(x = DATE, y = dissolvedOxygen)) +
  geom_point() # checking missing data: only 3 days of DO data for 2017 -> drop!
