...
# NEON - MCDI - Fish electrofishing, gill netting, and fyke netting counts (https://doi.org/10.48443/ap3d-rp07)
# SCALES/ SSECR                  
# Sierra Perez   
...
data %>% 
  filter(fishTotalLength == 0) %>% dplyr::count() # 2 zeros
...
# most species have low abundances (mean = 34.84) and somewhat right skewed (max = 676.00)
...
data$taxonRank[data$taxonID == "LEPMAC"] <- "species"
data$taxonRank[data$taxonID == "NOTEXI"] <- "species"
data$taxonRank[data$taxonID == "GAMSP"] <- "genus"

data %>% 
  filter(taxonRank == "genus" | taxonRank == "family" | taxonRank == "order" | taxonRank == "phylum" |  taxonRank == "class") %>% dplyr::count() 
# 549 observations not ID'd to species 
...
temp_final <- temp_final %>% 
  filter(!YEAR == 2017) # drop 2017 b/c averages low b/c temp data only for Nov-Dec of that year
