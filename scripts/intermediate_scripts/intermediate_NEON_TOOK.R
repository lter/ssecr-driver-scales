...
# NEON - TOOK - Fish electrofishing, gill netting, and fyke netting counts (https://doi.org/10.48443/kb2e-va82)
# SCALES/ SSECR                  
# Sierra Perez   
...
data %>% 
  filter(fishTotalLength == 0) %>% dplyr::count() # 1 0-length fish
...
unique(substr(data$date, 6, 7)) # months sampled: "07" "09" "06"
...
summary(count_check) # mean = 5.275; max = 101.000

data %>% 
  filter(taxonRank == "genus" | taxonRank == "family" | taxonRank == "order" | taxonRank == "phylum" |  taxonRank == "class") %>% dplyr::count() 
# 0 observations only ID'd to family, genus, order, or phylum -> for "ACTSPP" looks like sometimes they didn't bother sorting PIMPRO & CULINC to species
...
ggplot(temp, aes(x = DATE, y = surfacewaterTempMean)) +
  geom_point() # checking for missing data: looks great!
...
ggplot(DO, aes(x = DATE, y = dissolvedOxygen)) +
  geom_point() # checking missing data

# large periods of missing DO data, but averages fairly consistent across years
# keeping all data may want to consider dropping all DO data do to high missingness
